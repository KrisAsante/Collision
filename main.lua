...
local KoroSensei = display.newImage( "./assets/sprites/koro.png" )
KoroSensei.x = display.contentCenterX - 200
KoroSensei.y = display.contentCenterY
KoroSensei.id = "the character"
physics.addBody( KoroSensei, "dynamic", { 
    density = 3.0, 
    friction = 0.5, 
    bounce = 0.3 
    } )
KoroSensei.isFixedRotation = true -- If you apply this property before the physics.addBody() command for the object, it will merely be treated as a property of the object like any other custom property and, in that case, it will not cause any physical change in terms of locking rotation.
...
function jumpButton:touch( event )
    if ( event.phase == "ended" ) then
        ...
        KoroSensei:setLinearVelocity( 0, -750 )
    end
...
function checkCharacterPosition( event )
    ...
    if KoroSensei.y > display.contentHeight + 500 then
        KoroSensei.x = display.contentCenterX - 200
        KoroSensei.y = display.contentCenterY
    end
end
...
KoroSensei.collision = characterCollision
KoroSensei:addEventListener( "collision" )